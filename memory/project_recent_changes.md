...
3. **New planet: Sun ("Helion")** — `isSun:true`, added to planetDefs, position `(22000, -16000)` in space. Hidden until `p.discovered=true`, which is set when the ship enters `radius + 2500` of the sun's space position. Ship bounds expanded to `spaceWidth+20000` and `-spaceHeight-18000` so the player can actually reach it. Hidden from main space render + minimap until discovered.
